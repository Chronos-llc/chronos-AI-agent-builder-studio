...
# Apply fixes
# Fix 1: events_url malformed extra quote before colon
fixed = re.sub(r'%22events_url%22%22%3A', '%22events_url%22%3A', encoded)

# Fix 2: owner's url field missing opening quote and has asterisk
# Replace %22url%22%3A%2A with %22url%22%3A%22https%3A (adds opening quote and fixes asterisk)
fixed = re.sub(r'%22url%22%3A%2A', '%22url%22%3A%22https%3A', fixed)

# Decode
decoded = urllib.parse.unquote(fixed)

# Verify snippet
idx = decoded.find('"url"')
if idx >= 0:
    print("URL field snippet:", repr(decoded[idx:idx+80]))

try:
    data = json.loads(decoded)
    print("\n✓ JSON parsed successfully!")
    # Print pretty structured summary
    print(json.dumps(data, indent=2))
except json.JSONDecodeError as e:
    print(f"\n✗ JSON parse error at {e.pos}: {e.msg}")
    print(decoded[max(0,e.pos-50):e.pos+50])
