...
# Print what the substitution produced around the url field
idx = fixed.find('Chronos-llc%22')
if idx >= 0:
    print("Encoded snippet around owner url field:")
    print(fixed[max(0,idx-100):idx+50])
    
# Decode and show
decoded = urllib.parse.unquote(fixed)
print("\nDecoded snippet around position 720-750:")
print(repr(decoded[720:750]))

# Now check the exact pattern around 'url'
url_pos = decoded.find('"url"')
if url_pos >= 0:
    print(f"\nFull url field: {decoded[url_pos:url_pos+80]}")
...
try:
    ...
    print(f"Context: {decoded[max(0,e.pos-30):e.pos+30]}")
