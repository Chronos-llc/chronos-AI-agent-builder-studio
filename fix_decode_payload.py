...
# The original encoded string has two issues:
# 1. "events_url""https" - extra quote before colon
# 2. "url":"*//api.github.com" - asterisk instead of "https:"
...
# Fix 1: Replace the malformed asterisk URL with proper https
fixed = encoded.replace('%2A%2F%2Fapi.github.com', '%2F%2Fapi.github.com')
# Actually the asterisk is in the URL field, let me be more precise:
# The pattern is %22url%22%3A%2A%2F%2Fapi.github.com - should be %22url%22%3Ahttps%3A%2F%2Fapi.github.com
fixed = re.sub(r'%22url%22%3A%2A%2F%2F', '%22url%22%3Ahttps%3A%2F%2F', fixed)

# Fix 2: Remove extra quote in events_url field
fixed = re.sub(r'%22events_url%22%22%3A', '%22events_url%22%3A', fixed)

# Also fix the same asterisk pattern for the owner URL field
fixed = re.sub(r'%22url%22%3A%2A%2F%2F', '%22url%22%3Ahttps%3A%2F%2F', fixed)
...
try:
    ...
    print(f"\nContext around position {e.pos}:")
    start = max(0, e.pos - 100)
    end = min(len(decoded), e.pos + 100)
